<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="date" v-if="type!='info'" label="离宿时间" prop="lisushijian">
            <el-date-picker
                format="yyyy 年 MM 月 dd 日"
                value-format="yyyy-MM-dd"
                v-model="ruleForm.lisushijian" 
                type="date"
                placeholder="离宿时间">
            </el-date-picker> 
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.lisushijian" label="离宿时间" prop="lisushijian">
              <el-input v-model="ruleForm.lisushijian" 
                placeholder="离宿时间" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item class="textarea" v-if="type!='info'" label="离宿申请" prop="lisushenqing">
                <el-input
                  style="min-width: 200px; max-width: 600px;"
                  type="textarea"
                  :rows="8"
                  placeholder="离宿申请"
                  v-model="ruleForm.lisushenqing" >
                </el-input>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.lisushenqing" label="离宿申请" prop="lisushenqing">
                    <span>{{ruleForm.lisushenqing}}</span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
  ...
  data() {
	...
	userid : false,
      ...
      rules: {
          ...
          lisushijian: [
                { required: true, message: '离宿时间不能为空', trigger: 'blur' },
          ],
          lisushenqing: [
          ],
          shenqingriqi: [
  ...
  methods: {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `lisushenqing/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.lisushenqingCrossAddOrUpdateFlag = false;
                  this.parent.search();
